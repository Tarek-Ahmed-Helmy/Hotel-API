...
public class BasicDataController : ControllerBase
{
    ...
    {
        if (company == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "Company cannot be null." : "خطأ في الإدخال" });
    ...
    public async Task<IActionResult> GetCompany(string lang, int id)
    {
        ...
        if (company == null) return NotFound(new { Message = lang.ToLower() == "en" ? "Company not found." : "الشركة غير موجودة" });
    ...
    public async Task<IActionResult> GetAllCompanies(string lang)
    {
        ...
        if (companies == null || !companies.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No companies found." : "لم يتم ايجاد شركات" });
    ...
    {
        if (country == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "Country cannot be null." : "خطأ في الإدخال" });
    ...
    public async Task<IActionResult> GetCountry(string lang, int id)
    {
        ...
        if (country == null) return NotFound(new { Message = lang.ToLower() == "en" ? "Country not found." : "الدولة غير موجودة" });
    ...
    public async Task<IActionResult> GetAllCountries(string lang)
    {
        ...
        if (countries == null || !countries.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No countries found." : "لم يتم ايجاد دول" });
    ...
    {
        if (department == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "Department cannot be null." : "خطأ في الإدخال" });
    ...
    public async Task<IActionResult> GetDepartment(string lang, int id)
    {
        ...
        if (department == null) return NotFound(new { Message = lang.ToLower() == "en" ? "Department not found." : "القسم غير موجود" });
    ...
    public async Task<IActionResult> GetAllDepartments(string lang)
    {
        ...
        if (departments == null || !departments.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No departments found." : "لم يتم ايجاد اقسام" });
    ...
    {
        if (hotel == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "Hotel cannot be null." : "خطأ في الإدخال" });
    ...
    public async Task<IActionResult> GetHotel(string lang, int id)
    {
        ...
        if (hotel == null) return NotFound(new { Message = lang.ToLower() == "en" ? "Hotel not found." : "الفندق / العمارة غير موجودة" });
    ...
    public async Task<IActionResult> GetAllHotels(string lang)
    {
        ...
        if (hotels == null || !hotels.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No hotels found." : "لم يتم ايجاد فنادق" });
    ...
    {
        if (room == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "Room cannot be null." : "خطأ في الإدخال" });
    ...
    public async Task<IActionResult> GetRoom(string lang, int id)
    {
        ...
        if (room == null) return NotFound(new { Message = lang.ToLower() == "en" ? "Room not found." : "الغرفة غير موجودة" });
    ...
    public async Task<IActionResult> GetAllRooms(string lang)
    {
        ...
        if (rooms == null || !rooms.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No rooms found." : "لم يتم ايجاد غرف" });
    ...
    {
        if (roomType == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "RoomType cannot be null." : "خطأ في الإدخال" });
    ...
    public async Task<IActionResult> GetRoomType(string lang, int id)
    {
        ...
        if (roomType == null) return NotFound(new { Message = lang.ToLower() == "en" ? "RoomType not found." : "نوع الغرفة غير موجود" });
    ...
    public async Task<IActionResult> GetAllRoomTypes(string lang)
    {
        ...
        if (roomTypes == null || !roomTypes.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No room types found." : "لم يتم ايجاد انواع غرف" });
    ...
    public async Task<IActionResult> GenerateRoomTypes(string lang)
    {
        ...
        {
            Id = r.Id,
            Name = lang.ToLower() == "en" ? r.NameEn : r.NameAr,
        }).ToList();
        return Ok(roomTypeDtos);
    }
    ...
    {
        if (service == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "Service cannot be null." : "خطأ في الإدخال" });
        var lastService = (await _unitOfWork.Service.GetAllAsync()).OrderByDescending(s => s.Id).FirstOrDefault();

        var newService = new Service
        {
            Id = lastService == null ? 1 : lastService.Id + 1,
            NameAr = service.NameAr,
            NameEn = service.NameEn,
            ServiceLevel = service.ServiceLevel,
            SuperServiceId = service.SuperServiceId == 0 ? null : service.SuperServiceId
        };
    ...
    public async Task<IActionResult> GetService(string lang, int id)
    {
        ...
        if (service == null) return NotFound(new { Message = lang.ToLower() == "en" ? "Service not found." : "الخدمة غير موجودة" });
    ...
    public async Task<IActionResult> GetAllServices(string lang)
    {
        ...
        if (services == null || !services.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No services found." : "لم يتم ايجاد خدمات" });
    ...
    public async Task<IActionResult> GenerateServices(string lang)
    {
        ...
        if (level2Services == null || !level2Services.Any())
            return NotFound(new { Message = lang.ToLower() == "en" ? "No services found." : "لم يتم ايجاد خدمات" });
        ...
        foreach (var level2 in level2Services)
        {
            ...
            if (!grouped.ContainsKey(parent.Id))
            {
                ...
                {
                    Id = parent.Id,
                    Name = lang.ToLower() == "en" ? parent.NameEn : parent.NameAr,
            ...
            {
                Id = level2.Id,
                Name = lang.ToLower() == "en" ? level2.NameEn : level2.NameAr,
                SubServices = childs
                    .Select(child => new SelectServiceDTO
                    {
                        Id = child.Id,
                        Name = lang.ToLower() == "en" ? child.NameEn : child.NameAr
                    })
                    .ToList()
            };
    ...
    {
        if (status == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "Status cannot be null." : "خطأ في الإدخال" });
    ...
    public async Task<IActionResult> GetStatus(string lang, int id)
    {
        ...
        if (status == null) return NotFound(new { Message = lang.ToLower() == "en" ? "Status not found." : "الحالة غير موجودة" });
    ...
    public async Task<IActionResult> GetAllStatuses(string lang)
    {
        ...
        if (statuses == null || !statuses.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No statuses found." : "لم يتم ايجاد حالات" });
    ...
    {
        if (useType == null) return BadRequest(new { Message = lang.ToLower() == "en" ? "UseType cannot be null." : "خطأ في الإدخال" });
    ...
    public async Task<IActionResult> GetUseType(string lang, int id)
    {
        ...
        if (useType == null) return NotFound(new { Message = lang.ToLower() == "en" ? "UseType not found." : "نوع الاستخدام غير موجود" });
    ...
    public async Task<IActionResult> GetAllUseTypes(string lang)
    {
        ...
        if (useTypes == null || !useTypes.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No use types found." : "لا يوجد انواع استخدام" });
    ...
    {
        var useTypes = await _unitOfWork.UseType.GetAllAsync();
        if (useTypes == null || !useTypes.Any()) return NotFound(new { Message = lang.ToLower() == "en" ? "No use types found." : "لم يتم ايجاد انواع استخدام" });
        var useTypeDtos = useTypes.Select(u => new SelectUseTypeDTO
        {
            Id = u.Id,
            Name = lang.ToLower() == "en" ? u.NameEn : u.NameAr
        }).ToList();
